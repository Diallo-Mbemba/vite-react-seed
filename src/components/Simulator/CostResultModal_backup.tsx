...
const CostResultModal: React.FC<CostResultModalProps> = ({ result, onClose }) => {
  ...
  const [productCoefficients, setProductCoefficients] = useState<{[key: string]: number}>({});
  ...
  const handleCoefficientChange = (productId: string, newCoefficient: number) => {
    setProductCoefficients(prev => ({
      ...prev,
      [productId]: newCoefficient
    }));
  };
    ...
    if (chartRef.current && chartData.length > 0) {
      ...
      chartData.forEach((item) => {
        ...
        ctx.stroke();
                ...
                // Ajouter le pourcentage au centre de chaque section
        if (item.percentage >= 3) { // Afficher seulement si la section est assez grande
          const midAngle = currentAngle + sliceAngle / 2;
          const textRadius = radius * 0.7; // Position du texte à 70% du rayon
  ...
  const productData = (result.items || []).map((article: any) => {
    ...
    const valeurFOBTotale = result.items?.reduce((sum: number, item: any) => {
    ...
    // Droits et taxes par TEC (calculés sur la base FOB de l'article)
    ...
    if (tec && valeurFOBArticle > 0) {
      droitDouaneUnitaire = (valeurFOBArticle * (tec.dd || 0) / 100) / quantite;
      rstaUnitaire = (valeurFOBArticle * (tec.rsta || 0) / 100) / quantite;
      pcsUnitaire = (valeurFOBArticle * (tec.pcs || 0) / 100) / quantite;
      puaUnitaire = (valeurFOBArticle * (tec.pua || 0) / 100) / quantite;
      pccUnitaire = (valeurFOBArticle * (tec.pcc || 0) / 100) / quantite;
      rrrUnitaire = (valeurFOBArticle * (tec.rrr || 0) / 100) / quantite;
      rcpUnitaire = (valeurFOBArticle * (tec.rcp || 0) / 100) / quantite;
      tvaUnitaire = (valeurFOBArticle * (tec.tva || 0) / 100) / quantite;
      tsbUnitaire = (valeurFOBArticle * (tec.tsb || 0) / 100) / quantite;
      tabUnitaire = (valeurFOBArticle * (tec.tab || 0) / 100) / quantite;
    }
    const transitaireUnitaire = (result.prestationTransitaire * proportionFOB) / quantite;
    ...
    // Coefficient modifiable (par défaut 1.3)
    const coefficient = productCoefficients[article.id] || 1.3;
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="text-center py-4 px-4 text-gray-700 font-semibold">Code SH</th>
                ...
                <th className="text-right py-4 px-4 text-gray-700 font-semibold">DD/U</th>
                <th className="text-right py-4 px-4 text-gray-700 font-semibold">RSTA/U</th>
                <th className="text-right py-4 px-4 text-gray-700 font-semibold">PCS/U</th>
                <th className="text-right py-4 px-4 text-gray-700 font-semibold">PUA/U</th>
                <th className="text-right py-4 px-4 text-gray-700 font-semibold">PCC/U</th>
                <th className="text-right py-4 px-4 text-gray-700 font-semibold">RRR/U</th>
                <th className="text-right py-4 px-4 text-gray-700 font-semibold">RCP/U</th>
                <th className="text-right py-4 px-4 text-gray-700 font-semibold">TVA/U</th>
                <th className="text-right py-4 px-4 text-gray-700 font-semibold">TSB/U</th>
                <th className="text-right py-4 px-4 text-gray-700 font-semibold">TAB/U</th>
            ...
            <tbody>
              {productData.map((product: any, index: number) => (
                <tr key={product.id} className="border-b border-cote-ivoire-light hover:bg-cote-ivoire-lighter/30">
                  <td className="py-4 px-4 text-center text-gray-900 font-mono text-xs">{product.codeHS}</td>
                  ...
                  <td className="py-4 px-4 text-right text-gray-900">{formatCurrency(product.rstaUnitaire)}</td>
                  <td className="py-4 px-4 text-right text-gray-900">{formatCurrency(product.pcsUnitaire)}</td>
                  <td className="py-4 px-4 text-right text-gray-900">{formatCurrency(product.puaUnitaire)}</td>
                  <td className="py-4 px-4 text-right text-gray-900">{formatCurrency(product.pccUnitaire)}</td>
                  <td className="py-4 px-4 text-right text-gray-900">{formatCurrency(product.rrrUnitaire)}</td>
                  <td className="py-4 px-4 text-right text-gray-900">{formatCurrency(product.rcpUnitaire)}</td>
                  <td className="py-4 px-4 text-right text-gray-900">{formatCurrency(product.tvaUnitaire)}</td>
                  <td className="py-4 px-4 text-right text-gray-900">{formatCurrency(product.tsbUnitaire)}</td>
                  <td className="py-4 px-4 text-right text-gray-900">{formatCurrency(product.tabUnitaire)}</td>
                  ...
                  <td className="py-4 px-4 text-center">
                    <input
                      ...
                      onChange={(e) => handleCoefficientChange(product.id, parseFloat(e.target.value) || 1.3)}
                      className="w-16 px-2 py-1 text-center text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-cote-ivoire-primary focus:border-transparent"
                    />
                  </td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-2 text-sm text-gray-800">
              ...
              <p><strong>Marge %</strong> = (PV - PRU) / PV x 100. Modifiez le coefficient directement dans le tableau pour ajuster la marge.</p>
              <p><strong>Code SH</strong> = Code du Système Harmonisé pour la classification des marchandises.</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="mt-3 p-2 bg-blue-50 border border-blue-200 rounded text-xs text-blue-800">
                💡 <strong>Astuce :</strong> Cliquez sur le coefficient dans le tableau pour le modifier et ajuster la marge en temps réel.
              </div>
            </div>
          </div>
        </div>
  ...
  const renderAdminDecisionsTab = () => {
    ...
    const testDecisions = [
      {
        category: 'Test',
        title: 'Test de décision',
        description: 'Ceci est un test pour vérifier que les décisions s\'affichent',
        type: 'info' as const,
        icon: '🧪'
      },
      {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-green-600 mt-1">
              {testDecisions.filter(d => d.type === 'success').length}
            </p>
          </div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-yellow-600 mt-1">
              {testDecisions.filter(d => d.type === 'warning').length}
            </p>
          </div>
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-red-600 mt-1">
              {testDecisions.filter(d => d.type === 'error').length}
            </p>
          </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-blue-600 mt-1">
              {testDecisions.filter(d => d.type === 'info').length}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {testDecisions.map((decision, index) => (
            <div key={index} className="bg-white rounded-lg border border-gray-200 p-4">
              <h4 className="text-lg font-semibold text-gray-900 mb-2">{decision.title}</h4>
              <p className="text-gray-700">{decision.icon} {decision.description}</p>
            </div>
          ))}
        </div>
  ...
  return (
    ...
    console.log('Decisions by category:', decisionsByCategory);
    ...
    return (
      ...
      console.error('❌ ERREUR DANS RENDER ADMIN DECISIONS TAB:', error);
      return (
